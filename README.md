...
- **🖥️ macOS 原生灵动岛**：常驻实时字幕、悬停控制卡、完整工作台与独立 AI 焦点阅读器
...
麦克风和系统音频是两条独立的语义通道，转写分别标记为“我”和“会议”。任一来源不可用时，另一来源仍可继续。可在“设置 → 采集”关闭未来会议的本机麦克风；关闭后不会请求麦克风权限或启动 AVAudioEngine。灵动岛和工作台会在开始前显示有效选择，并提供暂停、继续和结束控制。暂停只停止音频采集，截图、会议历史和问答上下文仍保留。
...
每场会议使用独立的版本 2 时间线持久化转写、截图、截图分析、问题、回答、摘要、待办、来源和生命周期。工作台左侧按时间显示会议输入证据，右侧保留支持 Markdown、选择和复制的 AI 对话。一到三个不重复的“猜你想问”只在完整新结果成功时原子替换。结束后可重新打开历史会议并继续提问。
...
首次开始会议时按系统提示授权需要的屏幕录制与麦克风。拒绝麦克风权限不会中断系统音频，可从来源状态旁打开系统隐私设置并重试。先点“选择窗口”保留截图目标，之后每次点“截图”都会立即截取当前窗口，不会重复打开选择器。自动摘要与待办默认按第 5、10 分钟及以后每 5 个有效录音分钟生成，可配置为关闭、3、5 或 10 分钟。生成应用包：

“设置 → AI 服务”统一列出对话回答、猜你想问、摘要与待办、截图分析、实时翻译五个工作流。每项可独立选择 DeepSeek 或 OpenAI 兼容提供方并手动输入非空模型标识，提供方凭据和 Base URL 复用。新的 DeepSeek 选择使用项目既有的 `deepseek-chat` 默认值，但不限制前缀，未来或兼容端点的自定义模型标识仍可使用。OpenAI 兼容端点仅允许 HTTPS 或精确本机回环 HTTP；DeepSeek 端点要求 HTTPS。API Key 只保存在 Keychain。截图工作流若未声明视觉能力，会保留截图并明确记录模型未看到像素，不会伪造分析。
