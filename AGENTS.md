...
- Provider terminal markers end stream iteration, native answer deltas are coalesced per request before UI publication, and long reader sizing never performs repeated full-document measurement.
...
- Local PCM passes through the per-source adaptive gate in `SpeechActivityGate` before Whisper. Auxiliary loopback persistence coalesces about one second per source independently from raw-frame transcription; format changes flush and pause or stop discards incomplete batches. Pause and stop invalidate queued transcription work; macOS Voice Processing applies only to microphone capture and safely falls back when unsupported.
