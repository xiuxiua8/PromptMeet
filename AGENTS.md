...
- The island subtitle is a bounded accumulating stream (`SubtitleStreamFlow`): finals append to the buffer and flow at a speed that tracks the measured generation rate (points of content entering per second, including translations), so subtitles scroll past as fast as they are produced and the backlog never grows; a drain term clears bursts and the stream never restarts, never overwrites the page being read, and fully traversed pages drop off. The hover card renders the same buffered pages with auto-follow.
