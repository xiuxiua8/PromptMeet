...
- Provider terminal markers and an absolute completion deadline end stream iteration, native answer deltas are coalesced per request before UI publication, and long reader sizing never performs repeated full-document measurement.
...
- AI configuration saves during active or paused meetings defer companion reload until meeting completion. Meeting-history deletion stays disabled throughout those meetings.
