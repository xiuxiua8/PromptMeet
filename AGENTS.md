...
- Summary and task automation uses pause-aware active recording milestones, serializes generation per meeting, persists exact per-event character progress for budgeted chunks, carries complete structured prior state, rechecks source coverage before append, and stores append-only revisions.
