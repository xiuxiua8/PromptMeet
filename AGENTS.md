...
- `开始新会议` requires explicit confirmation when a meeting is active. A canonical durable meeting UUID is assigned before capture startup; backend creation may bind that same UUID immediately or after companion recovery.
...
- Summary and task automation uses pause-aware active recording milestones, serializes generation per meeting, reserves a fixed nonzero evidence allocation, persists exact per-event character progress for budgeted chunks, merges complete structured prior state outside that allocation, rechecks source coverage before append, and stores append-only revisions.
