...
class NotionTool(BaseTool):
    ...
    async def execute(self, action: str, **parameters) -> ToolResult:
        ...
        try:
            ...
            if action == "search":
                ...
                return ToolResult(
                    tool_name=self.name, success=False, error=f"不支持的操作: {action}"
                )
    ...
    async def _search(
        ...
        try:
                    ...
                    if item["object"] == "page":
                        ...
                        for prop_name, prop_value in properties.items():
                            if prop_value.get("type") == "title":
                                ...
                                if title_texts:
                                    item_info["title"] = title_texts[0].get(
                                        "plain_text", ""
                                    )
                                break
            ...
            return ToolResult(
                tool_name=self.name, success=False, error=f"搜索操作失败: {str(e)}"
            )
    ...
    async def _get_page(self, page_id: str) -> ToolResult:
        ...
        try:
            ...
            # 提取文本内容
            content_text = self._extract_text_from_blocks(
                blocks_data.get("results", [])
            )
            ...
            return ToolResult(
                tool_name=self.name, success=False, error=f"获取页面内容失败: {str(e)}"
            )

    async def _create_page(
        self, parent_id: str, title: str, content: Optional[str] = None
    ) -> ToolResult:
        """创建新页面"""
        try:
            ...
            return ToolResult(
                tool_name=self.name, success=False, error=f"创建页面操作失败: {str(e)}"
            )
    ...
    async def _update_page(self, page_id: str, **properties) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(
                tool_name=self.name, success=False, error=f"更新页面操作失败: {str(e)}"
            )
            ...
            if response.status_code == 200:
                ...
                for item in data.get("results", []):
                    ...
                    for prop_name, prop_value in item.get("properties", {}).items():
                        item_info["properties"][prop_name] = (
                            self._extract_property_value(prop_value)
                        )
    ...
    async def _create_database(
        self, parent_id: str, title: str, properties: Dict
    ) -> ToolResult:
        """创建数据库"""
        try:
