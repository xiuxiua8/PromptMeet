...
class FeishuCalendarTool(BaseTool):
    ...
    async def execute(
        self, result_file_path: Optional[str] = None, manual_task: Optional[dict] = None
    ) -> ToolResult:
        try:
            if manual_task:
                ...
                success, event_info = self._create_single_event(
                    client, calendar_id, user_access_token, manual_task
                )
                if success:
                    return ToolResult(
                        tool_name=self.name,
            ...
            if result_file_path is None:
                ...
                result_file_path = os.path.join(
                    project_root, "agents", "temp", "Result.txt"
                )
            ...
            for task in tasks:
                success, event_info = self._create_single_event(
                    client, calendar_id, user_access_token, task
                )
                if success:
                    success_count += 1
            ...
            return ToolResult(tool_name=self.name, result=result_info, success=True)
        except Exception as e:
            return ToolResult(
                tool_name=self.name,
                result={
                    "file_path": result_file_path,
                    "error": f"飞书日历同步失败: {str(e)}",
                    "type": "error",
                },
                success=False,
                error=str(e),
    ...
    def _extract_tasks_from_content(self, content: str) -> list:
        ...
        try:
            ...
            json_match = re.search(
                r"【结构化待办事项】\s*```json\s*({.*?})\s*```", content, re.DOTALL
            )

            if not json_match:
                # 尝试匹配没有```json标记的情况
                json_match = re.search(
                    r"【结构化待办事项】\s*({.*?})\s*(?=【|$)", content, re.DOTALL
                )
    ...
    def _create_single_event(
        self, client, calendar_id: str, user_access_token: str, task: Dict[str, Any]
    ) -> tuple:
        """创建单个日程事件"""
        try:
            ...
            if time_info["is_timed"]:
                event_description += (
                    f"时间: {time_info['start_time']} - {time_info['end_time']}"
                )
            else:
    ...
    def _parse_datetime_info(
        self, deadline: str, description: str = "", title: str = ""
    ) -> Dict:
        """解析日期时间信息，支持具体时间段和全天事件"""
        # 先获取基础日期
        base_date = (
            self._parse_relative_date(deadline)
            if not self._is_date_format(deadline)
            else deadline
        )
        ...
        for pattern in time_patterns:
            ...
            if match:
                time_info = self._extract_time_from_match(
                    match, pattern, base_date, full_text
                )
                if time_info["is_timed"]:
                    break
    ...
    def _extract_time_from_match(
        self, match, pattern: str, base_date: str, full_text: str
    ) -> Dict:
        """从正则匹配中提取时间信息"""
        groups = match.groups()
        ...
        try:
            ...
            # X点到Y点模式
                ...
                if len(groups) >= 2:
                    ...
                    # 调整AM/PM
                    start_hour, end_hour = self._adjust_ampm_hours(
                        start_hour, end_hour, full_text
                    )
            ...
            # 单个时间点模式
                ...
                end_dt = base_dt.replace(hour=hour, minute=minute) + timedelta(
                    hours=1
                )  # 默认1小时
