...
class TranslatorTool(BaseTool):
    ...
    def __init__(self):
        super().__init__(
            name="translate", description="文本翻译（使用MyMemory免费API）"
        )
    ...
    async def execute(self, text: str, target_lang: str = "en") -> ToolResult:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get("responseStatus") == 200:
                    ...
                    if "detectedLanguage" in data["responseData"]:
                        detected_lang = data["responseData"]["detectedLanguage"].get(
                            "language", source_lang
                        )
                        confidence = data["responseData"]["detectedLanguage"].get(
                            "confidence", 0
                        )
            ...
            return ToolResult(
                tool_name=self.name,
                result={"text": text, "error": f"翻译错误: {str(e)}", "type": "error"},
                success=False,
                error=str(e),
            )
