            ...
            if len(self.qa_generator.segment_buffer) >= 3:
                ...
                async with self.qa_generator.question_queue.lock:
                    for (
                        qid,
                        question,
                    ) in self.qa_generator.question_queue.questions.items():
                        questions.append(
                            {
                                "id": qid,
                                "question": question,
                ...
                logger.info(
                    f"缓冲区未达到3段，当前: {len(self.qa_generator.segment_buffer)}"
                )
                return {
                    "success": True,
                    "questions": [],
                ...
                if response.status_code == 200:
                    ...
                    if session_data.get("success"):
                        ...
                        if transcript_segments:
                            logger.info(
                                f"获取到转录片段，数量: {len(transcript_segments)}"
                            )

                            # 处理转录片段
                            result = await self.process_transcript_segments(
                                transcript_segments
                            )
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        if session_data.get("success"):
                            ...
                            logger.info(
                                f"当前转录片段总数: {len(transcript_segments)}, 上次检查: {last_segment_count}"
                            )

                            # 检查是否有新的转录片段
                            if len(transcript_segments) > last_segment_count:
                                logger.info(
                                    f"检测到新的转录片段，当前总数: {len(transcript_segments)}"
                                )
                        ...
                        if line:
                            try:
                                ...
                                if command.command == "start":
                                    ...
                                    # 启动监听转录片段的任务
                                    asyncio.create_task(
                                        processor._monitor_transcripts_and_generate_questions(
                                            command.session_id
                                        )
                                    )
