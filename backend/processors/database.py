...
class MeetingSessionStorage:
    ...
    def initialize_database(self):
        ...
        try:
            ...
            # 创建数据库(如果不存在)
            cursor.execute(
                "CREATE DATABASE IF NOT EXISTS meeting_sessions CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci"
            )
    ...
    def _update_existing_session(self, cursor, session_data):
        ...
        try:
            ...
            if "transcript_segments" in session:
                cursor.execute(
                    "DELETE FROM transcript_segments WHERE session_id = %s",
                    (session_id,),
                )
            ...
            if "current_summary" in session and session["current_summary"] is not None:
                # 删除旧的摘要相关数据
                cursor.execute(
                    "DELETE FROM session_summaries WHERE session_id = %s", (session_id,)
                )
                cursor.execute("DELETE FROM tasks WHERE session_id = %s", (session_id,))
                cursor.execute(
                    "DELETE FROM key_points WHERE session_id = %s", (session_id,)
                )
                cursor.execute(
                    "DELETE FROM decisions WHERE session_id = %s", (session_id,)
                )
    ...
    def _store_summary_data(self, cursor, summary):
        ...
        try:
            ...
            if "decisions" in summary and summary["decisions"]:
                ...
                decision_values = [
                    (session_id, decision) for decision in summary["decisions"]
                ]
                cursor.executemany(
                    """
        ...
        try:
            ...
            result = {
                ...
                "current_summary": (
                    {
                        "summary_text": (
                            summaries[0]["summary_text"] if summaries else ""
                        ),
                        "generated_at": (
                            summaries[0]["generated_at"] if summaries else None
                        ),
                        "tasks": [
                            {
                                "task": task["task_name"],
    ...
    def save_session_to_json_file(
        self,
        session_id: str,
        output_dir: str = os.path.join(".", "backend", "db", "session_data"),
    ) -> str:
