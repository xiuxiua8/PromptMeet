...
class WhisperProcessor:
            ...
            for i, dev in enumerate(devices):
                if dev["max_input_channels"] > 0:
                    ...
                    if sys.platform == "win32":
                        if (
                            "mix" in dev["name"].lower()
                            or "stereo" in dev["name"].lower()
                            or "混音" in dev["name"]
                        ):
                            self.device_id = i
    ...
    async def start_recording(self, session_id: str):
        ...
        try:
            ...
            logger.info(
                f"开始录音: 设备={device_info['name']}, 采样率={sample_rate}Hz, 通道数={channels}"
            )
            ...
            self.recording_thread = threading.Thread(
                target=self._recording_loop, args=(sample_rate, channels), daemon=True
            )
    ...
    async def _transcribe_audio(self, filename: str) -> Optional[TranscriptionResult]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Whisper API调用失败: {response.status_code} - {response.text}"
                )
                return None
    ...
    async def _send_transcription_result(self, result: TranscriptionResult):
        ...
        try:
            ...
            filter_words = [
                ...
                "歌曲",
                "詞曲",
            ]  # 注意：訂閱的"閱"是简繁体同形，这里使用"閱"的繁体形式            text = result.text or ""
            text = result.text or ""
            ...
            try:
                ...
                logger.debug(
                    f"繁体转简体完成: {text[:20]}... → {simplified_text[:20]}..."
                )
            except ImportError:
