...
class AudioTranscriber:
    ...
    def __init__(
        self, api_key: Optional[str] = None, chunk_size: int = 300, max_workers: int = 4
    ):
    ...
    def _split_audio(self, file_path: str) -> List[Tuple[str, float]]:
        ...
        if total_duration_sec <= self.chunk_size:
            print(
                f"🔍 直接处理完整文件 | 时长: {self._format_time(total_duration_sec)}"
            )
        ...
        for i in range(num_chunks):
            ...
            print(
                f"✂️ 分块 {i+1}/{num_chunks} | 时间范围: {start_sec:.1f}s-{end_sec:.1f}s"
            )

        return chunks

    def _transcribe_chunk(
        self, chunk_path: str, offset: float, model: str, keep_words: bool
    ):
        """转写单个分块（强制显示每个分块耗时）"""
    ...
    def transcribe_audio(
        self, file_path: str, model: str = "whisper-1", keep_words: bool = False
    ) -> Optional[Dict]:
        """执行多线程转写（显示每个分块独立耗时）"""
