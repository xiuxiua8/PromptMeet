...
class AudioPreprocessor:
    ...
    def __init__(self, output_dir: str = "processed_audio"):
        ...
        logger.info(
            f"音频预处理器初始化完成，输出目录: {os.path.abspath(self.output_dir)}"
        )

    def _validate_input(
        self, input_path: str, start_time: Optional[float], end_time: Optional[float]
    ):
        """验证输入参数有效性"""
    ...
    def _extract_audio_segment(
        self, audio: AudioFileClip, start: Optional[float], end: Optional[float]
    ) -> AudioFileClip:
        """提取音频片段"""
    ...
    def _process_chunk(
        self, chunk: np.ndarray, noise_sample: np.ndarray, sr: int
    ) -> np.ndarray:
        """处理单个音频块"""
        ...
        try:
            ...
            with AudioFileClip(input_path) as audio:
                ...
                if denoise:
                    ...
                    processed = []
                    for i in tqdm(
                        range(0, len(audio_data), chunk_size),
                        desc="降噪处理",
                        unit="chunk",
                    ):
