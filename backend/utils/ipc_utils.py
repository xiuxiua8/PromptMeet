...
class IPCManager:
    ...
    async def send_command(self, process_handle, command: IPCCommand, timeout: float = 30.0) -> IPCResponse:
        """发送IPC命令并等待响应"""
    ...
    def create_response(self, original_message: IPCMessage, success: bool, data: Any = None, error: str = None) -> IPCMessage:
        """创建响应消息"""
        response_data = {
...
def create_ipc_message(message_type: str, session_id: str, data: Dict[str, Any]) -> IPCMessage:
    """创建IPC消息"""
    return IPCMessage(
        message_id=str(uuid.uuid4()),
