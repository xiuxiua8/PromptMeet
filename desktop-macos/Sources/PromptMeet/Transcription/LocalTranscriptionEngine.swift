...
protocol LocalTranscriptionEngine: Sendable {
    ...
    /// Transcribe one utterance with an explicit whisper language hint
    /// ("auto" for per-utterance detection). Engines report their raw text plus
    /// whisper's own detection signal when available; gating happens upstream.
    func transcribe(
